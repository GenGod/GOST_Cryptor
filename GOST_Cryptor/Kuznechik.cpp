...
// Функция логирования
void print_byte(string str, unsigned char* value, unsigned int valueSize,
	string path)
{
	ofstream log;
	log.open(path.data(), ios::app);
	log << str << endl;
...
void print_byte(unsigned char* value, unsigned int valueSize, string path)
{
	ofstream log;
	log.open(path.data(), ios::app);
	for (int i = 0; i < valueSize; i++)
...
// Преобразование X
// "a" - входной параметр преобразования
// "b" - входной параметр преобразования
// "outdata" - результат преобразования
// "print" - функция логирования
// "return 0" - если всё преобразование прошло успешно
// "return -1" - если произошла ошибка
int makeX(unsigned char* a, unsigned char* b, unsigned char* outdata)
{
	unsigned int i;
	//string str = "XLog.txt";
	if (!a || !b || !outdata)
	{
		//print_byte("makeX: internal error!", 0, 0, str);
		return -1;
	}
	...
	//print_byte("makeX: a: ", a, 16, str);
	//print_byte("makeX: b: ", b, 16, str);
	//print_byte("makeX: result: ", outdata, 16, str);

	return 0;
}

// Преобразование S
// "indata" - входной параметр преобразования
// "outdata" - результат преобразования
// "print" - функция логирования
// "return 0" - если всё преобразование прошло успешно
// "return -1" - если произошла ошибка
int makeS(unsigned char* indata, unsigned char* outdata)
{
	size_t i = 0;
	...
	if (!indata || !outdata)
	{
		//print_byte("makeS: internal error!", 0, 0, str);
		return -1;
	}
	...
	//print_byte("makeS: input: ", indata, 16, str);
	//print_byte("makeS: output: ", outdata, 16, str);

	return 0;
}

// Преобразование, обратное к преобразованию  S
// "indata" - входной параметр преобразования
// "outdata" - результат преобразования
// "print" - функция логирования
// "return 0" - если всё преобразование прошло успешно
// "return -1" - если произошла ошибка
int makeReverseS(unsigned char* indata, unsigned char* outdata)
{
	unsigned int i;

	//string str = "ReverseSLog.txt";

	if (!indata || !outdata)
	{
		//print_byte("makeReverseS: internal error!", 0, 0, str);
		return -1;
	}
	...
	//print_byte("makeReverseS: input: ", indata, 16, str);
	//print_byte("makeReverseS: output: ", outdata, 16, str);
...
// "return 0" - если всё преобразование прошло успешно
// "return -1" - если произошла ошибка
int makeR(unsigned char* indata, unsigned char* outdata)
{
	size_t i;
	unsigned char sum = 0;
	...
	if (!indata || !outdata)
	{
		//print_byte("makeR: internal error!", 0, 0, str);
		return -1;
	}
	...
	outdata[0] = sum;
	memmove(outdata + 1, indata, 15);

	//print_byte("makeR: input: ", indata, 16, str);
	//print_byte("makeR: output: ", outdata, 16, str);
...
// "return 0" - если всё преобразование прошло успешно
// "return -1" - если произошла ошибка
int makeReverseR(unsigned char* indata, unsigned char* outdata)
{
	unsigned char tmp[16];
	unsigned char sum = 0;
	unsigned int i;

	//string str = "ReverseRLog.txt";

	if (!indata || !outdata)
	{
		//print_byte("makeReverseR: internal error!", 0, 0, str);
		return -1;
	}

	memmove(tmp, indata + 1, 15);
	tmp[15] = indata[0];
	...
	memmove(outdata, tmp, 15);
	outdata[15] = sum;

	//print_byte("makeReverseR: input: ", indata, 16, str);
	//print_byte("makeReverseR: output: ", outdata, 16, str);
...
// "return 0" - если всё преобразование прошло успешно
// "return -"1 - если произошла ошибка
int makeL(unsigned char* indata, unsigned char* outdata)
{
	unsigned char tmp[16];
	int i = 0;
	...
	if (!indata || !outdata)
	{
		//print_byte("makeL: internal error!", 0, 0, str);
		return -1;
	}

	memmove(tmp, indata, 16);

	for (i = 0; i < 16; ++i)
	{
		makeR(tmp, outdata);
		memmove(tmp, outdata, 16);
	}

	//print_byte("makeL: input: ", indata, 16, str);
	//print_byte("makeL: output: ", outdata, 16, str);

	return 0;
}

// Преобразование, обратное к преобразованию  L
// "indata" - входной параметр преобразования
// "outdata" - результат преобразования
// "print" - функция логирования
// "return 0" - если всё преобразование прошло успешно
// "return -1" - если произошла ошибка
int makeReverseL(unsigned char* indata, unsigned char* outdata)
{
	unsigned char tmp[16];
	unsigned int i;

	//string str = "ReverseLLog.txt";

	if (!indata || !outdata)
	{
		//print_byte("makeReverseL: internal error!", 0, 0, str);
		return -1;
	}
	...
	for (i = 0; i < 16; ++i)
	{
		makeReverseR(tmp, outdata);
		memmove(tmp, outdata, 16);
	}

	//print_byte("makeReverseL: input: ", indata, 16, str);
	//print_byte("makeReverseL: output: ", outdata, 16, str);
...
// "return 0" - если всё преобразование прошло успешно
// "return -1" - если произошла ошибка
int makeLSX(unsigned char* a, unsigned char* b, unsigned char* outdata)
{
	unsigned char temp1[16];
	unsigned char temp2[16];

	//string str = "LSXLog.txt";

	if (!a || !b || !outdata)
	{
		//print_byte("funcLSX: internal error!", 0, 0, str);
		return -1;
	}
	...
	//print_byte("funcLSX: a: ", a, 16, str);
	//print_byte("funcLSX: b: ", b, 16, str);
	//print_byte("funcLSX: output: ", outdata, 16, str);

	return 0;
}

// Преобразование, обратное к преобразованию  LSX
// "a" - входной параметр преобразования
// "b" - входной параметр преобразования
// "outdata" - результат преобразования
// "print" - функция логирования
// "return 0" - если всё преобразование прошло успешно
// "return -1" - если произошла ошибка
int makeReverseLSX(unsigned char* a, unsigned char* b, unsigned char* outdata)
{
	unsigned char temp1[16];
	unsigned char temp2[16];

	//string str = "ReverseLSXLog.txt";

	if (!a || !b || !outdata)
	{
		//print_byte("funcReverseLSX: internal error!", 0, 0, str);
		return -1;
	}

	makeX(a, b, temp1);
	makeReverseL(temp1, temp2);
	makeReverseS(temp2, outdata);

	//print_byte("funcReverseLSX: a: ", a, 16, str);
	//print_byte("funcReverseLSX: b: ", b, 16, str);
	//print_byte("funcReverseLSX: output: ", outdata, 16, str);

	return 0;
}

// Преобразование F
// "inputKey" - Первый ключ из пары ключей, полученной в предедущей итерации
// "inputKeySecond" - Второй ключ  из пары ключей, полученной в предедущей итерации
// "iterationConst" - Итерационная константа
// "outputKey" - Первый ключ
// "outputKeySecond" - Второй ключ
// "print" - функция логирования
// "return 0" - если всё преобразование прошло успешно
// "return -1" - если произошла ошибка
int makeF(unsigned char* inputKey, unsigned char* inputKeySecond,
	...
	unsigned char* outputKeySecond)
{
	...
	{
		//print_byte("funcF: internal error!", 0, 0, str);
		return -1;
	}

	makeLSX(inputKey, iterationConst, temp1);
	makeX(temp1, inputKeySecond, temp2);

	memmove(outputKeySecond, inputKey, 16);
	memmove(outputKey, temp2, 16);

	//print_byte("funcF: input key: ", inputKey, 16, str);
	//print_byte("funcF: input key: ", inputKeySecond, 16, str);
	//print_byte("funcF: iterration const: ", iterationConst, 16, str);
	//print_byte("funcF: output key: ", outputKey, 16, str);
	//print_byte("funcF: output key: ", outputKeySecond, 16, str);

	return 0;
}

// Вычисление итерационной константы С
// "number" - номер константы
// "output" - итерационная константа
// "print" функция логирования
// "return 0" если всё преобразование прошло успешно
// "return -1" - если произошла ошибка
int makeC(unsigned char number, unsigned char* output)
{
	...
	if (!output)
	{
		//print_byte("funcC: internal error!", 0, 0, str);
		return -1;
	}
...
// "keys" - массив развернутых ключей
// "print" - функция логирования
// "return 0" - если всё преобразование прошло успешно
// "return -1" - если произошла ошибка
int ExpandKey(const unsigned char* masterKey, unsigned char* keys)
{
	unsigned char C[16];
	unsigned char temp1[16];
	unsigned char temp2[16];
...
//	}
	//string str = "ExpandKeyLog.txt";

	if (!masterKey || !keys)
	{
		//print_byte("ExpandKey: internal error!", 0, 0, str);
		return -1;
	}

	memmove(keys, masterKey, 16);
	memmove(keys + 16, masterKey + 16, 16);

	//print_byte("ExpandKey: master key: ", masterKey, 32, str);
	//print_byte("ExpandKey: output key: ", keys, 16, str);
	//print_byte("ExpandKey: output key: ", keys + 16, 16, str);

	for (j = 0; j < 4; ++j)
	{
		memmove(temp1, keys + j * 2 * 16, 16);
		memmove(temp2, keys + (j * 2 + 1) * 16, 16);

		for (i = 1; i < 8; ++i)
		{
			makeC(j*8 + i, C);
			makeF(temp1, temp2, C, temp1, temp2);
		}

		makeC(j*8 + 8, C);
		makeF(temp1, temp2, C, temp1, temp2);

		memmove(keys + (j * 2 + 2) * 16, temp1, 16);
		memmove(keys + (j * 2 + 3) * 16, temp2, 16);

		//print_byte("ExpandKey: output key: ", keys + (j * 2 + 2) * 16, 16, str);
		//print_byte("ExpandKey: output key: ", keys + (j * 2 + 3) * 16, 16, str);
	}
	return 0;
}
...
// "return 0" - если всё преобразование прошло успешно
// "return -1" - если произошла ошибка
int Encrypt(unsigned char* plainText, unsigned char* keys)
{
	char str1[16];
	...
	//string str = "EncryptLog.txt";
	if (!plainText || !keys)
	{
		//print_byte("Encrypt: internal error!", 0, 0, str);
		return -1;
	}
	memmove(xTemp, plainText, 16);
	for (i = 0; i < 9; ++i)
	{
		makeLSX(xTemp, keys + 16*i, yTemp);
		memmove(xTemp, yTemp, 16);
	}
	makeX(yTemp, keys + 9*16, chipherText);
	for (i = 0; i < 10; ++i)
	{
		//print_byte("Encrypt: key: ", keys, 16, str);
		keys += 16;
	}
	//print_byte("Encrypt: plain text: ", plainText, 16, str);
	//print_byte("Encrypt: chipher text: ", chipherText, 16, str);
	//print_byte(chipherText, 16, "Encrypt.txt");
	memmove(plainText, chipherText, 16);
	//cout << endl;
	return 0;
}
...
// "return 0" - если всё преобразование прошло успешно
// "return -1" - если произошла ошибка
int Decrypt(unsigned char* chipherText, unsigned char* keys)
{
	unsigned char xTemp[16];
	unsigned char yTemp[16];
	...
	int i;
	char str1[16];
	//string str = "DecryptLog.txt";

	if (!plainText || !chipherText || !keys)
	{
		//print_byte("Decrypt: internal error!", 0, 0, str);
		return -1;
	}
	memmove(xTemp, chipherText, 16);
	for (i = 0; i < 9; ++i)
	{
		makeReverseLSX(xTemp, keys + (9 - i)*16, yTemp);
		memmove(xTemp, yTemp, 16);
	}
	makeX(yTemp, keys, plainText);
	for (i = 0; i < 10; ++i)
	{
		//print_byte("Decrypt: key: ", keys, 16, str);
		keys += 16;
	}
	//print_byte("Decrypt: chipher text : ", chipherText, 16, str);
	/*int nu = 0;
	int nuflag = 0;
	if (plainText[15] == '0')
	{
	...
	if (nuflag == 0)
	{
		//print_byte("Decrypt: plain text: ", plainText, 16, str);
		//print_byte(plainText, 16, "Decrypt.txt");
		memmove(chipherText, plainText, 16);
	}
	else
	{
		int g = 16 - nu;
		//print_byte("Decrypt: plain text: ", plainText, g, str);
		//print_byte(plainText, g, "Decrypt.txt");    */
		memmove(chipherText, plainText, 16);
	//}
	return 0;
}
...
int main()
{
	...
	ifstream ofs;
	int qwe = 0;
	string keyStr, keyStrC = "01234567890123456789012345678901";
	unsigned char plainText[16];
	...
	int h;
	while (1)
	{   cout<<"\n\t Текущий ключ: "<<keyStrC<<endl;
		cout << "1. Изменить ключ шифрования\n2.  Зашифровать текст из файла 'Text.exe'\n3. Расшифровать текст из файла 'Encrypt.exe' по известному ключу\n4. Выйти из программы\n"
			<< endl;
		cin >> h;
		if (h == 1)
		{
			cout << "Введите ключ, который Вы хотите использовать (32 символа): "
				<< endl;
			cin >> keyStrC;
		}
		else if (h == 2)
		{
			remove("Encrypt.exe");
			//cout << "Введите ключ, которым Вы хотите зашифровать текст (32 символа): "
			//	<< endl;
			//cin >> keyStr;
			keyStr=keyStrC;
			...
			FILE *f=fopen("Text.exe", "rb");
			FILE *fout=fopen("Encrypt.exe", "wb");
			...
			cout<<"!\n\tdone in-> "<<time<<" sec.\n\tsize of file-> "<<filesize<<" byte\n\tspeed-> "<<(filesize*8/time)/1024/1024<<" Mbit per sec."<<endl;
		}
		else if (h == 3)
		{
			remove("Decrypt.exe");
//			cout << "Введите ключ, которым Вы хотите расшифровать текст (32 символа): "
//				<< endl;
//			cin >> keyStr;
			keyStr=keyStrC;

			struct stat buf;
			stat("Encrypt.exe",&buf);
			int filesize=buf.st_size;
			...
			FILE *fin=fopen("Encrypt.exe", "rb");
			FILE *fout=fopen("Decrypt.exe", "wb");
		...
		{
			FILE *f1=fopen("Text.exe", "rb");
			FILE *f2=fopen("Decrypt.exe", "rb");
			int size=0;
			while (!feof(f1))
			{
				unsigned char chipherByte1[16],chipherByte2[16];
				fread(chipherByte1,sizeof(unsigned char),16,f1);
				fread(chipherByte2,sizeof(unsigned char),16,f2);

				for (int i = 0; i < 16; i++)
				{
					if (chipherByte1[i]!=chipherByte2[i])
					{
						cout<<"Error in "<<size<<" byte\n";
					}
					size++;
				}
	...
	cout << "Работа програмы завершена!" << endl;
	_getch();
	return 0;
}
